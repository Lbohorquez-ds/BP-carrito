...
async function session(){
  ...
  // Respaldo para el administrador principal: permite entrar al panel aunque
  // el perfil tarde en leerse. Las reglas de Firestore siguen protegiendo los datos.
  if(!CURRENT_ROLE && window.NDB_CONFIG?.adminUid === user.uid) CURRENT_ROLE='admin';
...
async function loadProducts(){
  ...
  // El catálogo es público: primero cargamos los productos sin exigir sesión.
  // También incluimos productos antiguos que todavía no tengan el campo `active`.
  try{
    const snap=await db.collection('productos').get();
    PRODUCTS=snap.docs.map(normalizeProduct)
      .filter(p=>p.active!==false)
      .sort((a,b)=>String(a.name||'').localeCompare(String(b.name||''),'es'));
  }catch(e){ console.error('No se pudo cargar el catálogo público:',e); PRODUCTS=[]; }
  // La sesión sólo define si se muestra la opción de compra; no condiciona la visualización.
  await session();
...
function productCard(p){
  ...
  const puedeComprar=!!CURRENT_USER;
  const boton=puedeComprar
    ...
    : `<a class="btn btn-red" href="${rootPrefix()}login.html?continuar=productos.html">Iniciar sesión para comprar</a>`;
  return `<article class="product">${p.on_sale?'<span class="badge">OFERTA</span>':''}${p.on_sale&&p.discount_percent?`<span class="discount">-${Number(p.discount_percent)}%</span>`:''}<img src="${esc(img)}" alt="${esc(p.name)}" onerror="this.src='${rootPrefix()}img/producto.jpg'"><div class="product-body"><div class="code">${esc(p.code||'')}${p.brand?' · '+esc(p.brand):''}</div><h3>${esc(p.name)}</h3><p class="muted">${esc(p.category||'Sin categoría')}</p><span class="stock ${sinStock?'agotado':''}">${sinStock?'AGOTADO':'Stock disponible: '+p.stock}</span><div class="price">${p.previous_price?`<span class="old-price">${money(p.previous_price)}</span>`:''}${money(p.price)}</div><div class="qty">${puedeComprar?`<input id="qty-${p.id}" type="number" min="1" max="${p.stock}" value="1" ${sinStock?'disabled':''}>`:''}${boton}</div></div></article>`;
}
...
async function deleteClient(id,name){if(!(await requireUser('admin')))return;if(!confirm(`¿Eliminar a ${name}?\n\nEl cliente perderá el acceso, pero sus pedidos históricos se conservarán.`))return;try{await db.collection('usuarios').doc(id).delete();ADMIN_CLIENTS=ADMIN_CLIENTS.filter(c=>c.id!==id);paintAdminClients();alert('Cliente eliminado. Ya no podrá ingresar a la tienda.');}catch(e){alert('No se pudo eliminar el cliente: '+e.message);}}
