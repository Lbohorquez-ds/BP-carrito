...
function productCard(p){
  ...
  const accion=puedeComprar
    ? `<div class="qty"><input id="qty-${p.id}" type="number" min="1" max="${p.stock}" value="1" ${sinStock?'disabled':''}><button class="btn btn-red btn-buy" onclick="addCart('${p.id}')" ${sinStock?'disabled':''}>${sinStock?'Sin stock':'Agregar'}</button></div>`
